...
# === Full Model Assembly without Temporal CNN+ ===
def build_full_model(input_shape, num_classes):
    inputs = Input(shape=input_shape)
    vit_branch = build_vision_transformer(input_shape)(inputs)
    # Directly pass Vision Transformer output to Attention Capsule Network
    final_output = build_attention_capsnet(vit_branch.shape[1:], num_classes)(vit_branch)
    return models.Model(inputs=inputs, outputs=final_output, name="ADMFO_CapsNet_ViT_Model_No_TCNN")
...
def build_attention_capsnet(input_shape, num_classes):
    inputs = layers.Input(shape=input_shape)
    attention = layers.Dense(input_shape[-1], activation='tanh')(inputs)
    attention = layers.Multiply()([inputs, attention])
    caps_output = CapsuleLayer(num_capsules=num_classes, dim_capsules=8)(attention)
    flat = layers.Flatten()(caps_output)
...
# === Full Model Assembly (WITHOUT ViT) ===
def build_full_model(input_shape, num_classes):
    inputs = layers.Input(shape=input_shape)
    tcnn_branch = build_temporal_cnn(input_shape)(inputs)
    final_output = build_attention_capsnet(tcnn_branch.shape[1:], num_classes)(tcnn_branch)
    return models.Model(inputs=inputs, outputs=final_output, name="ADMFO_CapsNet_No_ViT")
...
# === Full Model Assembly WITHOUT Capsule Network ===
def build_full_model_without_capsnet(input_shape, num_classes):
    inputs = Input(shape=input_shape)
    ...
    # Simple Dense classification head instead of CapsNet
    x = Dense(64, activation='relu')(concat)
    x = layers.Dropout(0.3)(x)
    output = Dense(num_classes, activation='softmax')(x)

    return models.Model(inputs=inputs, outputs=output, name="ADMFO_ViT_TCNN_NoCapsNet")

# === Build & Compile Model without Capsule Network ===
num_classes = y_categorical.shape[1]
model = build_full_model_without_capsnet((timesteps, features), num_classes)
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
model.summary()
...
# === Capsule Layer (Not used now, kept for reference) ===
class CapsuleLayer(layers.Layer):
    def __init__(self, num_capsules, dim_capsules, routing_iters=3, **kwargs):
        super(CapsuleLayer, self).__init__(**kwargs)
        self.num_capsules = num_capsules
...
# === Full Model Assembly without Attention CapsNet ===
def build_full_model_without_attention_capsnet(input_shape, num_classes):
    inputs = Input(shape=input_shape)
    vit_branch = build_vision_transformer(input_shape)(inputs)
    tcnn_branch = build_temporal_cnn(input_shape)(inputs)
    concat = Concatenate()([vit_branch, tcnn_branch])
    ...
    x = layers.Dropout(0.3)(x)
    output = Dense(num_classes, activation='softmax')(x)
    return models.Model(inputs=inputs, outputs=output, name="ViT_TCNN_Classifier")

# === Build & Compile Model without Attention CapsNet ===
num_classes = y_categorical.shape[1]
model = build_full_model_without_attention_capsnet((timesteps, features), num_classes)
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
model.summary()
